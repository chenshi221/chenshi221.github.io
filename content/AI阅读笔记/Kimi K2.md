...
**QK-Clip 机制**：对每个 attention head h，定义 max logit $S_{\max}^h = \frac{1}{\sqrt{d}}\max_{\mathbf{X}\in B}\max_{i,j}\mathbf{Q}_i^h\mathbf{K}_j^{h\top}$。当 $S_{\max}^h$ 超过阈值 $\tau$ 时，对投影权重做 per-head 缩放：

$$\mathbf{W}_q^h \leftarrow \gamma_h^{\alpha}\mathbf{W}_q^h, \quad \mathbf{W}_k^h \leftarrow \gamma_h^{1-\alpha}\mathbf{W}_k^h, \quad \gamma_h = \min(1, \tau/S_{\max}^h)$$

对 MLA 架构单独处理：$\mathbf{q}^C$ 和 $\mathbf{k}^C$ 各缩 $\sqrt{\gamma_h}$，$\mathbf{q}^R$ 缩 $\gamma_h$，共享的 $\mathbf{k}^R$ 不修改（避免跨头效应）。MuonClip = Muon + weight decay + consistent RMS + QK-Clip，全程 15.5T tokens 无 loss spike。
