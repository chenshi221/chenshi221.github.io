...
LLaMA 基于标准 Transformer（decoder-only），并集成了一系列改进：

| 架构选择 | 说明 | 相比标准 Transformer 的优势 |
| --- | --- | --- |
| **Pre-normalization (RMSNorm)** | LayerNorm 放在每个子层之前而非之后 | 提升训练稳定性（与 GPT-2 做法一致） |
| **SwiGLU 激活函数** | 替代 ReLU，用于 FFN 层 | 由 PaLM 验证，提升模型质量 |
| **Rotary Embeddings (RoPE)** | 替代绝对位置编码 | 更好的序列外推能力和相对位置感知 |

各模型规格：

| 模型 | 参数量 | 层数 | 隐藏维度 | 注意力头数 | 训练 tokens |
| --- | --- | --- | --- | --- | --- |
| LLaMA-7B | 6.7B | 32 | 4,096 | 32 | 1.0T |
| LLaMA-13B | 13.0B | 40 | 5,120 | 40 | 1.0T |
| LLaMA-33B | 32.5B | 60 | 6,656 | 52 | 1.4T |
| LLaMA-65B | 65.2B | 80 | 8,192 | 64 | 1.4T |
...
![](https://arxiv.org/html/2302.13971/x1.png)
