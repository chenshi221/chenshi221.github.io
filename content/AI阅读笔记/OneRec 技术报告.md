...
- 在快手真实用户数据上评估，AUC 提升 +0.5-1.2%（对比 DLRM 和 DCN-V2 等经典模型）
- 消融实验：
  - MoE vs Dense：~+0.3% AUC
  - RQ-KMeans vs 均匀哈希：~+0.2% AUC
  - 4-scale 特征：~+0.4% AUC
  - ECPO vs 基础模型：~+0.3% AUC

### 在线 A/B

在快手 App 首页信息流推荐：
- **单目标优化**：App 使用时长 +0.54%
- **多目标联合优化**：App 使用时长 +1.24%
- 同时提升了完播率、点赞率、评论率等互动指标
- 在 400M DAU 上稳定运行
